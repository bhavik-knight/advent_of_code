...
def main():
    ...
    for i in range(0, len(seeds), 2):
        if locations[i] < max_location:
            min_seed_index = i
    print(f"min_seed_index: {min_seed_index}")

    print(f"---", get_location(min_loc, lines[1:]))

    # start = seeds[min_seed_index]
    # end = start + seeds[min_seed_index + 1]
    # min_location = sys.maxsize
    # for seed in range(start, end):
    #     dst = get_location(seed, lines[1:])
    #     min_location = min(dst, min_location)
    #     print(seed, min_location)
